...
export class FuseSidebarComponent implements OnInit, OnDestroy
{
     ...
     */
    ngOnInit(): void {
        // Safe subscribe to config
        if (this._fuseConfigService?.config$?.pipe) {
            ...
            console.warn('[FuseSidebarComponent] config observable is not available.');
        }
        
    
        // Register sidebar only if name is valid
        if (this.name) {
            this._fuseSidebarService.register(this.name, this);
        } else {
            console.warn('[FuseSidebarComponent] Cannot register sidebar without a name.');
        }
    ...
    ngOnDestroy(): void {
        ...
        // Unregister only if name exists
        if (this.name) {
